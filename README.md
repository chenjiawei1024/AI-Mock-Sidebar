...
一个强大的Chrome浏览器扩展，用于快速生成、编辑和管理HCP系列项目的API模拟数据，支持AI生成模拟数据。
...
#### 可以通过两种途径获取初始数据模板
1. qam访问指定api详情界面
![生成模拟1](./images/guide/2.1生成模拟.png)
![生成模拟2](./images/guide/2.2生成模拟.png)
2. 直接使用真实环境的数据
![生成模拟3](./images/guide/2.3生成模拟.png)
![生成模拟4](./images/guide/2.4生成模拟.png)
...
2. 当模型可下载时，或不可用时，顶部会显示引导提示
3. 根据引导提示进行操作，跳转下载地址（在插件下载模型会与主页面冲突，导致状态有误，因此跳转到零码的地址进行下载）
4. 下载完成后，重新启动插件可看到ai模型可用的提示，此时便可以使用ai模拟功能。

#### 5.2 使用AI生成模拟

1. 填写好URL、请求方法和带注释的响应结构（注意，最好是带注释的字段例如"@ResponseStatus": "/* xs:object, note:响应状态 */",这样生成的效果更好）
2. 点击「AI 生成模拟」按钮
...
1. 如果是编辑url和请求类型，直接点击记录对应的标签即可进行原地编辑
2. 如果要编辑模拟数据，找到要编辑的模拟，点击「编辑」按钮
3. 进入编辑界面，修改模拟数据
4. 点击「保存模拟」按钮保存修改
